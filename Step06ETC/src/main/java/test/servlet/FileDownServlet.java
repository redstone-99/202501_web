...
public class FileDownServlet extends HttpServlet{
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//응답 헤더 정보 설정
		resp.setHeader("Content-Type", "application/octet-stream; charset=UTF-8");//1.파일을 전송할거야
		//다운로드 시켜줄 파일명 인코딩  
	   	String encodedName=URLEncoder.encode(orgFileName, "utf-8");
		//파일명에 공백이있는 경우 처리 
		encodedName=encodedName.replaceAll("\\+"," ");
		
		resp.setHeader("Content-Disposition", "attachment;filename="+encodedName);//2.파일명은 이거야
